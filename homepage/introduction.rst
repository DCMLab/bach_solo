...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/bach_solo.git
...
* `J.S. Bach – Solo Pieces (A corpus of annotated scores) <https://github.com/DCMLab/bach_solo/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first movement of the Violin Sonata No.1 in G minor has the following files:

-  ``MS3/BWV1001_01_Adagio.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/BWV1001_01_Adagio.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/BWV1001_01_Adagio.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/BWV1001_01_Adagio.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/BWV1001_01_Adagio.tsv')
   notes = ms3.load_tsv('notes/BWV1001_01_Adagio.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/bach_solo/issues>`__ and feel free to fork and submit pull requests.
